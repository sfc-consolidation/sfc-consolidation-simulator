 ...
 *
 * <p>
 * It creates the number of cloudlets defined in
 * {@link #CLOUDLETS}. All cloudlets will require 100% of PEs they are using all
 * the time.
 * Half of these cloudlets are created with the length defined by
 * {@link #CLOUDLET_LENGTH}
 ...
 *
 * <p>
 * However, you may notice in this case that the power usage isn't
 ...
 *
 * <p>
 * Realize that the Host CPU Utilization History is only stored
 * if VMs utilization history is enabled by calling
 * {@code vm.getUtilizationHistory().enable()}
 * </p>
 *
 * <p>
 * Each line in the table with CPU utilization and power consumption shows
 * the data from the time specified in the line, up to the time before the value
 * in the next line.
 * For instance, consider the scheduling interval is 10, the time in the first
 * line is 1 and
 ...
 * the CPU utilization and power consumption is the one provided in that first
 * line.
 * </p>
 *
...
public class Example3 {
   ...
   * For instance, if the CPU utilization mean of two equal VMs is 100% of their
   * CPU, the utilization
   * of each one corresponds to 50% of the Host's CPU utilization.</li>
   * <li>VM's power consumption relative to the total Host's power
   * consumption.</li>
   * </ul>
   *
   * <p>
   * A Host, even if idle, may consume a static amount of power.
   * Lets say it consumes 20 W in idle state and that for each 1% of CPU use it
   * consumes 1 W more.
   * For the 2 VMs of the example above, each one using 50% of CPU will consume 50
   * W.
   ...
   * This adds up to 70 W. If the two VMs are equal and using the same amount of
   * CPU,
   * their power consumption would be the half of the total Host's power
   * consumption.
   * This would be 60 W, not 70.
   * </p>
   *
   * <p>
   * This way, we have to compute VM power consumption by sharing a supposed Host
   * static power
  ...
  private List<Cloudlet> createCloudlets() {
    ...
    for (int i = 0; i < CLOUDLETS; i++) {
      ...
      final var cloudlet = new CloudletSimple(i, length, CLOUDLET_PES)
          .setFileSize(1024)
          .setOutputSize(1024)
