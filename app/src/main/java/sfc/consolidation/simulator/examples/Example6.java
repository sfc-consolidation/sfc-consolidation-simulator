...
public class Example6 {
  ...
  private void showSimulationResults() {
    ...
    for (NetworkHost host : datacenter.getHostList()) {
      System.out.printf("Host %d data transferred: %5d bytes, CPU: %6.2f, RAM: %6.2f, Power: %6.2f%n",
          host.getId(), host.getTotalDataTransferBytes(), host.getCpuUtilizationStats().getMean() * 100,
          host.getRam().getPercentUtilization() * 100,
          host.getPowerModel().getPower(host.getCpuUtilizationStats().getMean()));
    }
  ...
  private NetworkHost createHost() {
    ...
    PowerModelHost pm = new PowerModelHostSimple(800, 150);
  ...
  private List<NetworkCloudlet> createNetworkCloudlets() {
    ...
    NetworkDatacenter dc = createDatacenter();
    NetworkHost host = dc.getHostList().get(0);
    NetworkVm vm = new NetworkVm(-1, HOST_MIPS, HOST_PES);
    host.createVm(vm);
    NetworkCloudlet cloudlet = new NetworkCloudlet(TASK_LENGTH, HOST_PES);
    cloudlet
    ...
    addSendTask(cloudlet, netCloudletList.get(0));

    addReceiveTask(netCloudletList.get(0), cloudlet);
    addExecutionTask(netCloudletList.get(0));
    addSendTask(netCloudletList.get(0), netCloudletList.get(1));

    addReceiveTask(netCloudletList.get(1), netCloudletList.get(0));
    addExecutionTask(netCloudletList.get(1));
    addSendTask(netCloudletList.get(1), cloudlet);

    addReceiveTask(cloudlet, netCloudletList.get(1));
